...
def imshow_det_bboxes(img,
    ...
    for i, (bbox, label) in enumerate(zip(bboxes, labels)):
        ...
        if color_dict is not None:
            # _color = color_val_matplotlib(tuple(color_dict[label_text]))
            _color = [i / 255. for i in color_dict[label_text]]
            color.append(_color)
        else:
        ...
        if segms is not None:
            ...
            if color_dict is not None:
                # print(_color)
                _color = np.array(_color) * 255
                _color = _color.astype(np.uint8)
                img[mask] = img[mask] * 0.5 + _color * 0.5
            else:
